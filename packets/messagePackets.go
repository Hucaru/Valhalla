...
func MessageRedText(msg string) maplepacket.Packet {
	p := maplepacket.CreateWithOpcode(opcodes.Send.ChannelInfoMessage)
	p.WriteByte(9)
	p.WriteString(msg)
...
func MessageNotice(msg string) maplepacket.Packet {
	p := maplepacket.CreateWithOpcode(opcodes.Send.ChannelBroadcastMessage)
	p.WriteByte(0)
	p.WriteString(msg)
...
func MessageBubblessChat(msgType byte, sender string, msg string) maplepacket.Packet {
	p := maplepacket.CreateWithOpcode(opcodes.Send.ChannelBubblessChat)
...
func MessageWhisper(sender string, message string, channel byte) maplepacket.Packet {
	p := maplepacket.CreateWithOpcode(opcodes.Send.ChannelWhisper)
	p.WriteByte(0x12)
	p.WriteString(sender)
	p.WriteByte(channel)
...
func MessageAllChat(senderID int32, isAdmin bool, msg string) maplepacket.Packet {
	p := maplepacket.CreateWithOpcode(opcodes.Send.ChannelAllChatMsg)

...
func GetCharacterSkills(charID int32) map[int32]int32 {
	...
	// row, err := connection.Handle.Query("SELECT "+filter+" FROM skills WHERE characterID=?", charID)
...
func GetCharacter(charID int32) Character {
	...
	// err := connection.Handle.QueryRow("SELECT "+filter+" FROM characters where id=?", charID).Scan(&charID,
	// 	&userID,
	// 	&worldID,
	// 	&name,
	// 	&gender,
...
func GetCharacters(userID int32, worldID int32) []Character {
	...
	chars, err := database.Handle.Query("SELECT "+filter+" FROM characters WHERE accountID=? AND worldID=?", userID, worldID)

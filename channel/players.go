...
var Players = maplePlayers{players: make(map[mnet.MConnChannel]*MapleCharacter), mutex: &sync.RWMutex{}}
...
type maplePlayers struct {
	players map[mnet.MConnChannel]*MapleCharacter // keep as maps as it's sparse data?
	mutex   *sync.RWMutex
}

func (p *maplePlayers) AddPlayer(conn mnet.MConnChannel, char *character.Character) {
	p.mutex.Lock()
	p.players[conn] = &MapleCharacter{*char, conn}
	p.mutex.Unlock()
}

func (p *maplePlayers) RemovePlayer(conn mnet.MConnChannel) {
	p.mutex.Lock()
...
func (p *maplePlayers) OnCharacterFromConn(conn mnet.MConnChannel, action func(char *MapleCharacter)) {
	p.mutex.RLock()
	if _, exists := p.players[conn]; exists {

...
if (plr.job() === 0) {
    if (plr.getLevel() >= 10) {
        ...
        if (npc.sendYesNo("You look qualified for this. With sharp eyes and cold precision, a Thief strikes from the shadows. Do you want to become a #rThief#k?")) {
            ...
            npc.sendBackNext("Alright! You are a Thief from here on out... Here's a little bit of my power to you... Haahhhh!")
            npc.sendBackNext("I've added a few slots to your Equip and ETC inventories, and your strength has increased. Train hard. I'll be watching your every move, so don't let me down.")
    ...
    if (plr.getLevel() >= 30) {
        npc.sendNext("Hmmm... you seem to have gotten a whole lot stronger. Ready to take the next step?")
        ...
        while (chosen === -1) {
            ...
            if (npc.sendYesNo("So you want to advance as a #b" + jobName + "#k? Once you decide, you can't go back. Are you sure?")) {
                plr.setJob(jobId)
                if (branch === 0) {
                    npc.sendBackNext("Alright, from here on out you are the #bAssassin#k. Keep training and hone your skills in the shadows.")
                } else {
                    npc.sendBackNext("Alright, from here on out you are the #bBandit#k. Keep training and hone your skills in the shadows.")
                }

...
if (sel == 0) {
    ...
    if (sel == 0) {                                            // Pay 2000 Mesos
        if (npc.sendYesNo("You want to pay #b2000 Mesos#k to go to #m120030000#? Sure, but don't forget that there are monsters there, too. I'll prepare to set sail. Okay. You ready to head for #m120030000# right now?")) {
            if (plr.mesos() < 2000) {
                npc.sendBackNext("I think you're lacking mesos. There are many ways to gather up some money, you know, like... selling your armor... defeating monsters... doing quests... you know what I'm talking about.", false, true)
            } else {
                plr.giveMesos(-2000)
                plr.warp(110000000)
            }
        } else {
            npc.sendBackNext("You must have some business to take care of here. You must be tired from all that traveling and hunting. Go take some rest, and if you feel like changing your mind, then come talk to me.", false, true)
        }
        ...
        if (npc.sendYesNo("So you have a #bVIP Ticket to Florina Beach#k? You can always head over to Florina Beach with that. Alright then, but just be aware that you may be running into some monsters there too. Okay, would you like to head over to Florina Beach right now?")) {
            if (plr.itemCount(4031134) < 1) {
                npc.sendBackNext("Hmmm, so where exactly is your #bVIP Ticket to Florina Beach#k? Are you sure you have one? Please double-check.", false, true)
            } else {
                if (!plr.removeItemsByID(4031134, 1)) {
                    npc.sendBackNext("Something went wrong while taking your #bVIP Ticket to Florina Beach#k. Please try again.", false, true)
                } else {
                    plr.warp(110000000)
                }
            }

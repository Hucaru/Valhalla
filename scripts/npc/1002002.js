...
if (sel == 0) {
    ...
    if (sel == 0) {                                            // Pay 2000 Mesos
        ...
        if (npc.sendYesNo("So you have a #bVIP Ticket to Florina Beach#k? You can always head over to Florina Beach with that. Alright then, but just be aware that you may be running into some monsters there too. Okay, would you like to head over to Florina Beach right now?")) {
            if (plr.itemCount(4031134) < 1) {
                ...
                if (!plr.removeItemsByID(4031134, 1)) {
                    npc.sendNext("Something went wrong while taking your #bVIP Ticket to Florina Beach#k. Please try again.")
                } else {
                    plr.warp(110000000)
                }
            }
        ...
        npc.sendBack("I came back without it, and it just feels awful not having it. Hopefully someone picked it up and put it somewhere safe. Anyway, this is my story and who knows, you may be able to pick it up and put it to good use. If you have any questions, feel free to ask.")
    }
}
if (npc.sendYesNo("If you're looking for someone who can pinpoint the characteristics of various items, you're looking at him now. Would you like to hear my story?")) {
    ...
    if (plr.itemQuantity(item[selection]) < 100) {
        npc.sendOk("Hey, what do you think you're doing? Go lie to some other unassuming fellow. Not me!")
    } else {
        if (plr.getInventory(2).getNumFreeSlot() < 1) {
            npc.sendOk("What? I can't give you the reward if your Equip, Use, or Etc window in your Item Inventory is full. Please make room and I will gladly give you what you came for.")
        } else {
            var rand = Math.floor(Math.random() * Prizes.length)
            npc.sendOk(txt[rand])
            plr.takeItem(item[selection], 100)
            plr.giveItem(Prizes[rand], num[rand])
        }
    }
} else {

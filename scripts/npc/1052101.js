...
if (select == 0) {
    ...
    hair = hair[Math.floor(Math.random() * hair.length)] + (plr.hair() % 10)
    ...
    if (npc.sendYesNo("If you use the REG coupon your hair will change RANDOMLY with a chance to obtain a new experimental style that I came up with. Are you going to use #b#t5150052##k and really change your hairstyle?")) {
        if (plr.takeItem(5150052, 1)) {
            plr.setHair(hair)
            npc.sendBackNext("Ok, here's the mirror. Your new haircut! What do you think? I know it wasn't the smoothest, but it still looks pretty good! Come back later when you need to change it up again!", true, true)
        } else {
            npc.sendBackNext("Hmmm...are you sure you have our designated coupon? Sorry but no haircut without it.", true, true)
        }
    } else {
        npc.sendBackNext("I see...think about it a little more and if you want to do it, come talk to me.", true, true)
    }
} else if (select == 1) {
    // Dye
    var base = Math.floor(plr.hair() / 10) * 10
    var hair = [base + 0, base + 1, base + 2, base + 3, base + 4, base + 5]
    ...
    if (npc.sendYesNo("If you use a regular coupon your hair will change RANDOMLY. Do you still want to use #b#t5151035##k and change it up?")) {
        if (plr.takeItem(5151035, 1)) {
            plr.setHair(hair)
            npc.sendBackNext("Ok, here's the mirror. Your new haircolor! What do you think? I know it wasn't the smoothest, but it still looks pretty good! Come back later when you need to change it up again!", true, true)
        } else {

...
if (sel === 0) {
    npc.sendBackNext("I'm #p2040030#, and I research many types magic here in place of my master, #p1032102#. It seems there are a lot of pets in Ludibrium, as well. I'll excuse myself now, as I have plenty of pending tasks to attend to.")
} else if (sel === 1) {
    ...
    npc.sendBackNext("That's right! Pets can't eat the normal human food. Instead a teddy bear in Ludibrium called #b#p2041014##k sells #bPet Food#k so if you need food for your pet, find #b#p2041014##k It'll be a good idea to buy the food in advance and feed the pet before it gets really hungry.")
    npc.sendBackNext("Oh, and if you don't feed the pet for a long period of time, it goes back home by itself. You can take it out of its home and feed it but it's not really good for the pet's health, so try feeding him on a regular basis so it doesn't go down to that level, alright? I think this will do.")
} else if (sel === 2) {
    npc.sendNext("Dying... well, they aren't technically ALIVE per se, so I don't know if dying is the right term to use. They are dolls imbued with #p1012005#'s magical power and the Water of Life. Of course while they're animated, they act just like a living animal...")
    npc.sendBackNext("After enough time has passes, the Water of Life bringing your pets to life will run out and they'll go back to being dolls. But they won't stay like that forever! Just use some Premium Water of Life to revive them!")
    npc.sendBackNext("Even if it someday moves again, it's sad to see them stop altogether. Please be nice to them while they are alive and moving. Feed them well, too. Isn't it nice to know that there's something alive that follows and listens to only you?")
} else if (sel === 3) {
    ...
    if (npc.sendYesNo("250,000 mesos will be deducted. Do you really want to buy?")) {
        if (plr.mesos() < 250000 || !plr.giveItem(4160011, 1)) {
            ...
            npc.sendOk("Thank you for your purchase.")
        }
    }
}